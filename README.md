...
- 移植仓库：<https://github.com/ZHCOOL520/LoliPickaxe>（Minecraft 1.20.1 Forge，mod 版本 `1.0.1`）

> **版本号说明**：移植版使用本仓库自己的编号 **`1.0.1`**。
...
| 注册名 | 显示名 | 说明 |
| --- | --- | --- |
| `loli_altar` | 萝莉祭坛 | 按图案摆放后召唤萝莉，见 [3.2.1](#321-萝莉祭坛怎么摆) |
| `password_work_bench` | 密码工作台 | 带密码的合成台，见 [3.2.2](#322-密码工作台是什么) |
...
祭坛**不是合成出来的**，而是要求在世界上**按特定图案摆放祭坛方块**，
再用萝莉镐右键**中心那一格**来召唤萝莉。规则写在
`BlockLoliAltar#initAltarStructure()` 里，是一个 **63×63 的图案**：

| 项目 | 数值 |
| --- | --- |
| 图案尺寸 | **63 × 63** |
| 需要的祭坛方块 | **1169 个** |
| 中心 | 以**被右键的方块**为中心（数组下标 `[31][31]`） |
| 判定 | 全 63×63 必须**逐格完全匹配**，错一格都不会召唤 |

图案具有**四重旋转对称**：中心是一个不对称的 9×9 结构，向外是若干圈对称的环。
**中心那一格必须是祭坛方块**，玩家右键的就是它。

> **在 JEI 里查看**：JEI 新增了「**萝莉祭坛摆放方式**」类别，
> 会把整张 63×63 缩略图与中心 9×9 放大图并排画出来。
> 点击「萝莉祭坛」方块即可跳转到该页（JEI 中搜 `@lolipickaxe` 也能找到）。

召唤后祭坛方块会被**清除**（`level.removeBlock`），所以每次召唤都要重摆。

#### 3.2.2 密码工作台是什么

它是一个**需要额外输入密码的合成台**：界面上除了 3×3 材料格，
还有一个**密码输入框**（`GUIPasswordCrafting`）。合成判定时调用
`PasswordRecipeManager.findMatchingResult(craftMatrix, player, password)`
—— **材料摆对还不够，密码也必须对**，否则产物永远是空的。

**但要注意一个事实**：本模组**自身没有注册任何密码配方**。

```
PasswordRecipeLoader.register(...) 的调用点：0 个
IPasswordRecipe 的实现类：0 个
代码/资源中硬编码的密码：无
```

查证 1.12.2 原版后确认，**原版也是如此**：它注册的是一个
`disableSaving().allowModification()` 的**空配方表**，
`IPasswordRecipe` 只是一个**留给整合包作者实现的接口**：
...
**结论**：密码工作台是**面向整合包/扩展模组的 API 钩子**，
不是一个开箱即用的合成台。要让它真正产出物品，
必须有**另一个模组**实现 `IPasswordRecipe` 并调用 `PasswordRecipeLoader.register(...)`
把配方注册进来。密码值本身也由那个模组决定。

> 因此 JEI 里对密码工作台的说明只写了"需要输密码、密码由整合包或服务器预设"，
> **没有编造任何具体密码**。
...
- **蓝屏 / 崩溃 / 未响应打击**：三种特效攻击**均已实现**，但实现方式与原版有本质区别，逐项说明如下。

  | 攻击 | 1.12.2 原版 | 本移植版 | 差异性质 |
  | --- | --- | --- | --- |
  | **蓝屏** | 内嵌并 `Runtime.exec` 运行一个 38.7 KB 的 `BlueScreen.exe` | 游戏内仿真蓝屏界面（`GUILoliBlueScreen`） | **安全化改写** |
  | **崩溃** | `FMLCommonHandler.exitJava(0, true)` | `Runtime.halt(0)`，且限定仅在客户端进程执行 | 行为等价 |
  | **未响应** | `while(true);` 开满所有 CPU 核心 + 主线程死循环 | 阻塞客户端主线程一个**有限时长**（默认 15 秒） | **安全化改写** |

  **为什么蓝屏改为仿真界面**：对原版内嵌的那个可执行文件做静态分析（提取字节 → `MZ` 头 → 导入表）后确认，
  它调用 `ADVAPI32!LookupPrivilegeValueA` / `AdjustTokenPrivileges` 为自己开启 `SeShutdownPrivilege` 关机特权，
  再调用 `ntdll!ZwRaiseHardError` —— 这是触发**真实操作系统蓝屏（BSOD）**的未公开 NT 调用。
  其作用是在**任意一台安装了本模组的电脑上强制崩溃宿主操作系统**，会导致未保存数据丢失，
  且影响范围超出游戏本身。**本移植版不内嵌、不写盘、不执行该文件**，
  改为游戏内仿真界面：观感一致（标准蓝底、`:(`、进度动画、停止代码、仿真二维码），
  进度结束后真实断开服务器连接，但**宿主系统完全不受影响**。

  **为什么未响应改为限时**：原版的 `while(true);` 会占满所有 CPU 核心并把整机拖死，
  玩家连"关闭程序"都点不动。现改为只阻塞客户端主线程一个有界时长，
  足以让操作系统把窗口标记为「未响应」并显示幽灵窗口，但系统整体仍可用，玩家可正常关闭。
  时长由 `loliFailRespondDuration` 控制（默认 15000 毫秒，范围 0 ~ 120000）。

  > 原版三种攻击都只作用于**客户端**；本移植版同样如此，
  > 专用服务端收到该包会直接忽略（代码中有 `FMLEnvironment.dist` 判定）。

- **跨世界传送黑名单**：配置项仍为数字维度 id，1.20.1 维度标识为 `ResourceKey`，仅原版三个维度可命中。
- **萝莉镐展示框渲染**：不再替换原版 `ItemFrame` 渲染器，配置项 `loliCardRenderFrame` 不再生效。
- **刷怪蛋**：已补齐 `loli_spawn_egg`（对应 1.12.2 的 `EntityRegistry.registerEgg`，
  颜色同为 `0xFFFFFF` / `0x000000`），并已加入创造模式物品栏。
  > 1.12.2 只给 `loli` 注册了刷怪蛋，`loli_buff_attack_tnt` 本来就没有，这一点保持一致。
...
`com.anotherstar.compat.jei` 包提供 **6 个** JEI 类别：
...
| **萝莉祭坛摆放方式** | **63×63 建造图案缩略图 + 中心 9×9 放大图 + 图例** |

并为以下物品提供 **信息页**（鼠标悬停显示获取途径与当前配置下的实际概率）：
氪金萝莉、普通萝莉、生物灵魂、萝莉卡片、卡片册、唱片、三色炸弹、
**密码工作台、萝莉祭坛方块**。

> **祭坛类别**的数据来自 `LoliAltarPattern`，它是 `BlockLoliAltar` 中
> 63×63 图案的**只读副本**，含 `selfCheck()` 自检（行数 / 每行长度 / 方块总数 1169 / 中心格）。
> 若以后修改了 `BlockLoliAltar` 的图案，**必须同步更新 `LoliAltarPattern`**。
...
7. **布局按类别分工**：3 列工作台类配方用 `layout()`（输入网格 + 箭头 + 输出）；
   单输入类配方（生物掉落）用 `layoutSingle()` 紧凑排布，
   避免通用布局的固定列宽在只有 1 个输入时留出 2 格空洞造成"错位"。
...
- **修复小熊/小萝莉镐自动收纳物品后「收进去了但打开储藏室看不到」**。
  根因：`getPage()` 返回的是容器**内部**的 `NonNullList`，直接 `set`/`grow` 虽能改到内存，
  却不经过 `setChanged()`，该页因此不会被标记为脏页；而 `stopOpen()` 对**未变脏**的页
  会复用读入时的原始 NBT 快照，把刚收进来的物品整份覆盖掉。
  现统一改为经由 `ILoliInventory#insertItem` 写穿（内部 `setItemForPage` → `markDirty`）。
  > 该问题在 1.0 中已修过 `DestroyBedrockEvent` 一处，本版补齐了
  > `SmallLoliBlockDropEvent` 中**同样写法**的另一处。

**修复（稳定性类）**

- **修复 `/loli <配置名> <值>` 对 INT 配置完全不做边界钳位的问题**。
  该命令原先直接写入解析值、无视注解声明的上下界，已证实的后果包括：
  - `loliPickaxeMaxPage` 设成极大值 → `getPage` 按需分配页时会尝试分配海量对象，**服务端 OOM**
  - `loliPickaxeSlotStackLimit` 设成过大值 → 堆叠数相加**整数溢出为负**，物品数量错乱

  现新增 `clampIntFlag()` 统一按注解钳位；同时为
  `loliPickaxeMaxPage`（`1 ~ 10000`）与
  `loliPickaxeSlotStackLimit`（`1 ~ 2000000000`）补上显式上下界。
  > **未声明上界的配置项行为不变** —— `intMaxValue` 的默认值 `0` 语义是「不限制上界」，
  > 实现中已按此处理，不会把未声明上界的字段误夹成 `0`。

- **修复容器 `updateSlot()` 的越界风险**：其上界原先取 `getContainerSize()` 而未校验
  `slots.size()`，现改为两者取小，与同文件 `broadcastChanges()` 的防护保持一致。

**默认值调整**

- `loliPickaxeAutoAccept` 默认值 `true` → **`false`**（默认不把掉落物收进储藏室）
  > 老存档的 `config/lolipickaxe-common.toml` 已存在时，Forge **不会**用新默认值覆盖它，
  > 需手动改或删掉该文件重新生成。

**修复（资源类）**

- **修复升级音效永远加载不出来**：`sounds.json` 声明的是 `lolisuccess`，
  而文件名是 `loliSuccess.ogg`（大写 `S`）。MC 的资源路径**区分大小写**，
  因此该音效从未被加载，日志中一直有 `File lolipickaxe:sounds/lolisuccess.ogg does not exist`。
  已将文件改名为 `lolisuccess.ogg`。

**JEI 增强**

- **新增「萝莉祭坛摆放方式」JEI 类别**。此前玩家只能靠一张图片猜摆法，
  而那张图（`lolicards/召唤祭坛摆放方式.png`）因文件名含中文不会被资源管理器加载，
  等于**根本没地方查**。新类别把 63×63 图案的缩略图与中心 9×9 放大图并排画出，
  并用图例标注颜色含义；点击「萝莉祭坛」方块即可跳转。
- **修复「生物掉落」界面错位**。根因：该类别只有 1 个输入，
  但沿用了为 3 列工作台设计的固定布局常量（输出槽固定在第 3 列之后），
  于是来源与产物之间空出约 2 格。现为该类别单独实现紧凑布局
  （宽度 110px → 74px），并把概率文字从硬编码的 `getHeight() - 14`
  改为按槽位推导的固定间距。
- **补充信息页**：密码工作台、萝莉祭坛方块。

**文档**

- README 新增 [3.2.1 萝莉祭坛怎么摆](#321-萝莉祭坛怎么摆) 与
  [3.2.2 密码工作台是什么](#322-密码工作台是什么)。
  密码工作台部分**明确说明本模组自身不注册任何密码配方**，
  它是留给整合包作者实现 `IPasswordRecipe` 的 API 钩子（1.12.2 原版亦然），
  **未编造任何具体密码**。

> **关于卡片文件名**：`lolicards/` 下的 `小莫女儿''N.png` 与
> `召唤祭坛摆放方式.png` 会触发 `Invalid path in pack ... ignoring` 日志，
> 但**这是预期行为、不是 bug**：`''` 是**卡册分组的分隔符**
> （`ItemLoliCardAlbum` 用 `indexOf('\'')` 解析组名），
> 且 `LoliCardUtil` 已专门绕开资源管理器直接读取这些文件。
> **不要重命名它们**，否则卡册分组会失效、老存档的卡册也将无法匹配。
...
- 萝莉刷怪蛋（`loli_spawn_egg`），并补齐进创造模式物品栏
- 三种「特效攻击」的**游戏内**实现（蓝屏界面 / 未响应卡顿 / 终止客户端进程），
  详见第四节的「已知的行为差异 / 降级」
